...
| --------------- | ------------------------ | ---------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Primary         | `llmapi.withcortex.ai`   | DigitalOcean App Platform, NYC, two replicas   | `new_api` database on `cortex-prod` PostgreSQL 16 and the existing `redis-prod` Redis 7 cluster |
| Hong Kong route | `llmapicn.withcortex.ai` | Existing Hong Kong server, Caddy reverse proxy | Uses the primary PostgreSQL and Redis data plane through `llmapi.withcortex.ai`                 |

There is one application deployment and one authoritative data plane. The Hong
Kong endpoint is a stateless acceleration route to the primary endpoint;
...
5. Confirm that the existing HK Caddy host can reach
   `https://llmapi.withcortex.ai/api/status`.
...
existing Cortex production shape: GitHub-driven Dockerfile builds, two 1-vCPU,
1-GB NYC App Platform replicas, PostgreSQL 16, Redis 7, health checks,
deployment/domain alerts, and automatic deploys from `main`.

Set `SESSION_SECRET` as an encrypted runtime environment variable on the
`new-api-intl` service before sending production traffic. If the existing
...
and any live non-`withcortex.ai` custom domains into an untracked temporary
copy of the spec and apply that copy. Never write the generated secret or its
encrypted value to the repository. After the update, confirm that
`SESSION_SECRET` is still present with type `SECRET` and that the retained
external domains are still attached before sending traffic.

## Hong Kong site

Add the `deploy/cortex/hk/Caddyfile` site block to the existing HK Caddy
configuration. It terminates TLS for `llmapicn.withcortex.ai`, preserves the
client-facing forwarded host, and sends the origin request to
`https://llmapi.withcortex.ai` with the correct TLS server name and Host header.
...
Point the DNS record for `llmapicn.withcortex.ai` at the existing HK server
...
The canonical `llmapi.withcortex.ai` hostname keeps this homepage.
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GitHub       | Register `https://llmapi.withcortex.ai/oauth/github`. GitHub's documented callback matching also permits the `llmapicn.withcortex.ai` subdomain route when the callback path matches. See [GitHub's redirect URL rules](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps#redirect-urls). |
| Discord      | Register both exact callbacks: `https://llmapi.withcortex.ai/oauth/discord` and `https://llmapicn.withcortex.ai/oauth/discord`. [Discord applications expose a `redirect_uris` list](https://docs.discord.com/developers/resources/application#application-object-application-structure). |
| OIDC         | Register both exact callbacks: `https://llmapi.withcortex.ai/oauth/oidc` and `https://llmapicn.withcortex.ai/oauth/oidc`. [OIDC clients define a `redirect_uris` array](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata); every used value must be registered exactly. |
| LinuxDO      | Register only `https://llmapi.withcortex.ai/oauth/linuxdo`. The gateway does not offer LinuxDO login or binding on trusted aliases because this deployment has one client configuration and no checked multi-callback capability. |
| Custom OAuth | Replace `<slug>` in `https://llmapi.withcortex.ai/oauth/<slug>` and register that canonical callback. Custom providers are not offered on trusted aliases because each provider has one client configuration and no per-origin capability setting. |

Keep the two origins in `.do/new-api-intl.yaml`'s
...
on the trusted Hong Kong route; LinuxDO and custom OAuth providers are visible only
...
The logo URL uses the canonical `llmapi.withcortex.ai` hostname. While DNS is
...
curl -fsS https://llmapicn.withcortex.ai/api/status
```

Both responses must include `"success":true` and report the same New API
version and setup state. Complete the setup wizard once through the primary
endpoint, then create a low-quota test token and verify the same token through
both routes:
...
curl -fsS https://llmapicn.withcortex.ai/v1/models \

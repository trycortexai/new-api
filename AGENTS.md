...
- Non-required CI failures may be treated as an accepted upstream baseline only when the same failure signature has already been confirmed on the current upstream baseline and all tests and checks affected by the change pass. Do not re-investigate an unchanged baseline signature on every PR; re-investigate if the signature changes or overlaps the current work. Required checks are never bypassable.

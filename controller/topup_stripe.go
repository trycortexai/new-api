...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	payLink, err := genStripeLink(referenceId, user.StripeCustomer, user.Email, req.Amount, req.SuccessURL, req.CancelURL, c.Request.Host)
	if err != nil {
...
func genStripeLink(referenceId string, customerId string, email string, amount int64, successURL string, cancelURL string, requestHost string) (string, error) {
	...
	if successURL == "" {
		successURL = paymentReturnPathForHost(requestHost, "/usage-logs")
	}
	if cancelURL == "" {
		cancelURL = paymentReturnPathForHost(requestHost, "/wallet")
	}
